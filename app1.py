...
# =========================
# 0) 全域設定 & 路徑（Google Drive）
# =========================
st.set_page_config(
    page_title="年增率 + 財務比率分析儀表板（只讀雲端版）",
    layout="wide"
)
...
def robust_read_csv(src: str, **kwargs) -> pd.DataFrame:
    ...
    # 若呼叫端有指定 encoding，先取出放到 encodings 第一個
    first_enc = kwargs.pop("encoding", None)
    encodings = [first_enc, "utf-8-sig", "utf-8", "big5", "cp950"]
...
def load_yoy_data(src: str) -> pd.DataFrame:
    ...
    yoy_cols = [
        ...
        if ("年增率" in c) and (not str(c).strip().startswith("平均"))
    ]
...
def load_financial_ratios(url_gm: str, url_om: str, url_cf: str) -> pd.DataFrame:
    ...
    for df_ in (gm_m, om_m, cf_m):
        if not df_.empty:
            df_["更新狀態"] = status_flag

    df_fin = gm_m.merge(
        om_m[["代號", "名稱", "日期", "營益率"]] if not om_m.empty else
        pd.DataFrame(columns=["代號", "名稱", "日期", "營益率"]),
        on=["代號", "名稱", "日期"], how="outer"
    ).merge(
        cf_m[["代號", "名稱", "日期", "營業金流"]] if not cf_m.empty else
        pd.DataFrame(columns=["代號", "名稱", "日期", "營業金流"]),
        on=["代號", "名稱", "日期"], how="outer"
    )
...
def get_latest_with_fallback(df, year):
    ...
    if not df_prev.empty:
        ...
        st.warning(
            f"⚠️ {year} 年資料未公布，已沿用上一季（{last_prev['日期'].iloc[0].strftime('%Y-%m-%d')}）"
        )
        return last_prev
    return pd.DataFrame()
...
if (show_fin or show_radar or show_radar_mix) and len(selected) == 1:
    ...
    years_fin = (
        ...
        if not fin_s.empty and "日期" in fin_s.columns
        else []
    )
    years_yoy = (
        sorted(yoy_s_single["日期"].dt.year.dropna().unique().tolist())
        if not yoy_s_single.empty and "日期" in yoy_s_single.columns
        else []
    )
        ...
        if chosen_years:
            ...
            for yr in chosen_years:
                ...
                if latest_fin.empty:
                    continue
                real_vals = [
                    float(latest_fin["毛利率"].values[0]) if "毛利率" in latest_fin.columns and pd.notna(latest_fin["毛利率"].values[0]) else 0.0,
                    float(latest_fin["營益率"].values[0]) if "營益率" in latest_fin.columns and pd.notna(latest_fin["營益率"].values[0]) else 0.0,
                    float(latest_fin["營業金流"].values[0]) if "營業金流" in latest_fin.columns and pd.notna(latest_fin["營業金流"].values[0]) else 0.0,
                ]
                norm_vals = [
    ...
    if show_radar_mix:
        ...
        if chosen_years2:
            ...
            for yr in chosen_years2:
                ...
                real_vals = [
                    float(latest_fin["毛利率"].values[0]) if not latest_fin.empty and "毛利率" in latest_fin.columns and pd.notna(latest_fin["毛利率"].values[0]) else 0.0,
                    float(latest_fin["營益率"].values[0]) if not latest_fin.empty and "營益率" in latest_fin.columns and pd.notna(latest_fin["營益率"].values[0]) else 0.0,
                    float(latest_fin["營業金流"].values[0]) if not latest_fin.empty and "營業金流" in latest_fin.columns and pd.notna(latest_fin["營業金流"].values[0]) else 0.0,
                    float(latest_yoy["年增率"].values[0]) if not latest_yoy.empty and "年增率" in latest_yoy.columns and pd.notna(latest_yoy["年增率"].values[0]) else 0.0,
                ]
                norm_vals = [
...
with st.expander("🏆 平均年增率排行榜 Top 10", True):
    try:
        ...
        avg_col = next(
            (c for c in df_raw.columns if c.strip() in ("平均年增率", "平均 年增率")),
            None,
        )
        if avg_col:
            df_avg = (
...
with st.expander(f"📈 近 {grow_n} 個月年增率連續成長", True):
    ...
    if result:
        ...
        industries = ["全部顯示"] + sorted(
            df_res["產業分類"].dropna().unique()
        )
...
# =========================
st.caption(
    "小提醒：年增率為『月資料』，財務比率為『季資料』；若最新季度尚未完整，圖表會暫以上一季沿用顯示。"
)
